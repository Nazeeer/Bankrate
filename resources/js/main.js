...
function calculateMonthlyPayment(monthlyInterestRate, numberOfPayments, presentValue) {

    var monthlyPayment = (monthlyInterestRate * presentValue) / (1 - Math.pow(1 + monthlyInterestRate, -numberOfPayments));
...
btn_calculate.addEventListener('click', function(){
    ...
    let payment = calculateMonthlyPayment(rate, months, Number.parseInt(Home_price.value))
    ...
    while(x < months){
        ...
        while(i < 7){
            ...
            if(i == 1){
                td.innerText = new_date.toLocaleDateString('default', {month: 'short'}) + ' ' + new_date.getFullYear();
            }
            if(i == 2)
                td.innerText = formatter.format(Number.parseFloat(beginning))
            if(i == 3)
                td.innerText = formatter.format(Number.parseFloat(payment))
            if(i == 4)
                td.innerText = formatter.format(Number.parseFloat(interest))
            if(i == 5)
                td.innerText = formatter.format(Number.parseFloat(principal))
            if(i == 6){
